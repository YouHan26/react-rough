import EsPaintWorklet from 'css-paint-es';
import { solidFillPolygon, patternFillPolygon, polygon } from '../node_modules/roughjs/bin/renderer.js';
	...
	paint(ctx, geometry, properties) {
		...
		if (options.fill) {
			if (resolvedOptions.fillStyle === 'solid') {
				opSets.push(solidFillPolygon(points, resolvedOptions));
			} else {
				opSets.push(patternFillPolygon(points, resolvedOptions));
			}
		}
		if (options.strokeWidth && (options.strokeWidth > 0)) {
			opSets.push(polygon(points, resolvedOptions));
		}
